...
def Fitting_with_censored_data():

    ##### Data from cens_fit_data_norng.m #####
    ...
    y = X.T.dot(c_true) + 0.1*(np.sqrt(n))*np.array([
            [-0.8],[-0.46],[0.19],[0.89],[-1.6],[-0.32],[-0.7],[-0.74],[0.37],[1.4],
            [0.46],[0.66],[1.1],[0.98],[-1.3],[-0.023],[0.13],[2.4],[0.9],[0.076],
            [0.36],[-2.1],[-2.3],[-0.37],[1.3],[0.56],[-0.18],[-0.036],[1.9],[1.3],
            [-0.36],[-0.33],[0.083],[0.43],[-1.2],[-2.7],[-0.54],[2.2],[-0.61],[1.4],
            [1.2],[0.74],[-0.12],[-0.031],[-1],[-1],[0.63],[0.87],[2.1],[-0.59],
            [0.59],[1.5],[2.1],[-0.75],[-1.5],[-0.21],[0.49],[-0.32],[0.82],[-1.4],
            [1.1],[0.98],[0.49],[1.3],[0.59],[0.45],[-1.4],[-0.92],[-1.4],[-1.5],
            [0.097],[-0.23],[-0.32],[0.63],[0.039],[-0.016],[0.45],[-0.52],[-1.1],[-0.048],
            [-0.5],[1.6],[-0.24],[-0.96],[-0.15],[1.7],[1.6],[-1.1],[0.097],[0.25],
            [2.2],[0.31],[2],[-0.12],[-0.27],[1.4],[-0.074],[-0.36],[-0.78],[-0.25]])

    # Reorder measurements, then censor
    sort_ind = np.argsort(y, axis=0).flatten()
    y = np.sort(y, axis=0)

    X = X[:,sort_ind];
    D = (y[M-1]+y[M])/2; # Matlab is 1-indexed while numpy is 0-indexed
    y = y[0:M]; # Matlab is 1-indexed while numpy is 0-indexed
    ...
    def part1():
        c = cp.Variable((n,1))
        y_censored = cp.Variable((K-M,1))
        y_combined = cp.vstack([y_uncensored, y_censored])

        constraints = [y_censored >= D]
        J = cp.quad_form(y_combined - cp.matmul(X.T, c), np.identity(K))
        prob = cp.Problem(cp.Minimize(J), constraints)
        prob.solve()
        # print("c = " + str(c.value))
        residual = np.linalg.norm(c_true - c.value, ord=2) / np.linalg.norm(c_true, ord=2)
        print("residual = " + str(residual))

    def part2():
        c_ls = cp.Variable((n,1))
        J_ls = cp.quad_form(y_uncensored - cp.matmul(X[:,0:M].T, c_ls), np.identity(M))
        prob_ls = cp.Problem(cp.Minimize(J_ls))
        prob_ls.solve()
        # print("c_ls = " + str(c_ls.value))
        residual_ls = np.linalg.norm(c_true - c_ls.value, ord=2) / np.linalg.norm(c_true, ord=2)
        print("residual_ls = " + str(residual_ls))

    part1()
    part2()

def Minimax_rational_fit_to_exponential():
    k = 201
    t = [-3+6*(i-1)/(k-1) for i in range(1,k+1)]
    y = np.array([[np.e**ti] for ti in t])
    ...
    constraints = [1 + b1*ti + b2*ti**2 >= 0 for ti in t] 
    '''
    Naively express the problem as stated in the question
    '''
    # f = cp.bmat([[(a0 + a1*ti + a2*ti**2)/(1 + b1*ti + b2*ti**2)] for ti in t])
    # prob = cp.Problem(cp.Minimize(cp.max(cp.abs(f - y))), constraints)
    # prob.solve()
    '''
    The above fails as the objective is not convex
    '''

    '''
    Try converting (f - y) to (f_nominator - y*denominator)
    Also express cp.max(cp.abs(...)) as cp.norm_inf(...)
    '''
    # f_nominator = cp.bmat([[a0 + a1*ti + a2*ti**2] for ti in t])
    # f_denominator = cp.bmat([[1 + b1*ti + b2*ti**2] for ti in t])
    # prob = cp.Problem(
            # cp.Minimize(cp.max(cp.abs(f_nominator - cp.multiply(y,f_denominator)))),
            # constraints)
    ...
    # print("b1 = " + str(b1.value))
    # print("b2 = " + str(b2.value))
    # print("Optimal = " + str(prob.value))
    '''
    The above seems to also work, though not as rigorous as the correct solution
    '''

    '''
    Correct solution
    max|f - y| is not convex but quasiconvex,
    hence we have max|f - y| < z
    The correct solution is to convert the original problem of
    minimize max{i=1..k} |f - y| into
    minimize z
    s.t.
    z >= 0
    f_nominator_i - yi*f_denominator_i <= z*f_denominator_i for i=1..k
    f_nominator_i - yi*f_denominator_i >= -z*f_denominator_i for i=1..k
    '''
    # z = cp.Variable()
    # constraints = [ z>=0 ] + [
            # a0 + a1*t[i] + a2*t[i]**2 - y[i] * (1+b1*t[i] + b2*t[i]**2) <= z*(1+b1*t[i]+b2*t[i]**2) for i in range(len(t))] + [
            # a0 + a1*t[i] + a2*t[i]**2 - y[i] * (1+b1*t[i] + b2*t[i]**2) >= -z*(1+b1*t[i]+b2*t[i]**2) for i in range(len(t))]
    # prob = cp.Problem(cp.Minimize(z), constraints)
    '''
    Note that this is still not convex since it insolves the expression z*b1 and z*b2
    However, since this is the optimization of one variable,
    we can use a simple bisection method to find the minimum z
    '''
    z = cp.Parameter()
    constraints = (
            [a0 + a1*t[i] + a2*t[i]**2 - y[i] * (1+b1*t[i] + b2*t[i]**2) <= z*(1+b1*t[i]+b2*t[i]**2) for i in range(len(t))] + 
    ...
    while upper - lower >= epsilon:
        z.value = lower + (upper - lower)/2
        print("lower = " + str(lower))
        print("upper = " + str(upper))
        print("z = " + str(z.value))
        prob.solve(solver="ECOS") # Using ECOS yields answers closest to solution
    ...
    print("b1 = " + str(b1_opt))
    print("b2 = " + str(b2_opt))
    f = np.array([[(a0_opt + a1_opt*ti + a2_opt*ti**2)/(1 + b1_opt*ti + b2_opt*ti**2)] for ti in t])
    print("Optimal = " + str(np.max(np.abs(f - y))))

Fitting_with_censored_data()
Minimax_rational_fit_to_exponential()
